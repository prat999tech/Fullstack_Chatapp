...
function Signup() {
  ...
  const validatePasswordMatch = (value) => {
    ...
    const onSubmit = async (data) => {
        ...
        await axios.post("http://localhost:8001/api/v1/user/signup", userinfo)
        .then((response) => {
  ...
  return (
   ...
   <div className="max-w-4xl max-sm:max-w-lg mx-auto p-6 mt-6">
      <div className="text-center mb-12 sm:mb-16">
        ...
        <h4 className="text-white text-base mt-6">Sign up into your account</h4>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <div className="grid sm:grid-cols-2 gap-8">
          <div>
            ...
            <input name="name" type="text" className="bg-slate-100 w-full text-white text-sm px-4 py-3 rounded focus:bg-transparent outline-blue-500 transition-all" placeholder="fullname" {...register("fullname", { required: true })}/>
          </div>
          
          <div>
            <label className="text-white text-sm font-medium mb-2 block">Email Id</label>
            <input name="email" type="email" className="bg-slate-100 w-full text-white text-sm px-4 py-3 rounded focus:bg-transparent outline-blue-500 transition-all" placeholder="Enter email" {...register("email", { required: true })} />
          </div>
           <div>
           <label className="text-white text-sm font-medium mb-2 block">Confirm password</label>
            <input name="number" type="number" className="bg-slate-100 w-full text-white text-sm px-4 py-3 rounded focus:bg-transparent outline-blue-500 transition-all" placeholder="confirm password" {...register("confirm password", { required: true,validate:validatePasswordMatch })}/> 
          </div>
